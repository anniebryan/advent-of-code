...
def parse_outcome(outcome: str) -> "Outcome | Rule | str":
    if "," in outcome:
        return Rule(outcome)
    if outcome == "A":
...
class Rule:
    def __init__(self, rule: str):
        ...
        self.outcome_if_true = parse_outcome(outcome_if_true)
        self.outcome_if_false = parse_outcome(outcome_if_false)
    ...
    def execute(self, **kwargs: int) -> str | Outcome:
...
def num_combinations_accepted(
    ...
    min_val = kwargs[f"{rule.varname}_min"]
    max_val = kwargs[f"{rule.varname}_max"]
    ...
    if rule.outcome_if_true != Outcome.REJECT:
        ...
        if n > 0:
            if rule.outcome_if_true == Outcome.ACCEPT:
                num_combinations += n * prod(
                    max(kwargs[f"{v}_max"] - kwargs[f"{v}_min"] + 1, 0)
                    for v in "xmas" if v != rule.varname
                )
...
def solve_part_2(puzzle_input: list[str]):
    ...
    return num_combinations_accepted(
        ...
        s_min=MIN_VALUE, s_max=MAX_VALUE,
    )
